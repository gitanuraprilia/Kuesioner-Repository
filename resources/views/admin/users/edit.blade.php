            ...
            <div class="card-body">
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="stage" placeholder="{{ __('Mahasiswa/Dosen') }}" name="stage" value="{{ old('stage', $user->stage) }}" />
                    </div>
                    <div class="form-group">
                        <label for="nim_nidn_nik">{{ __('NIM/NIDN/NIK') }}</label>
                        <input type="text" class="form-control" id="nim_nidn_nik" placeholder="{{ __('NIM/NIDN/NIK') }}" name="nim_nidn_nik" value="{{ old('nim_nidn_nik', $user->nim_nidn_nik) }}" />
                    </div>
                    <div class="form-group">
                        <label for="study_program">{{ __('Program Studi') }}</label>
                        <input type="text" class="form-control" id="study_program" placeholder="{{ __('Program Studi') }}" name="study_program" value="{{ old('study_program', $user->study_program) }}" />
                    </div>
                    <div class="form-group">
                        <label for="system_access">{{ __('rekuensi Mengakses Sistem Digital Repository (Dalam 1 Bulan)') }}</label>
                        <input type="text" class="form-control" id="system_access" placeholder="{{ __('<4/4-6/7-9/10-12/>12') }}" name="system_access" value="{{ old('system_access', $user->system_access) }}" />
                    </div>
                    <div class="form-group">
